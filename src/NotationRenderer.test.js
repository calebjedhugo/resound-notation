...
describe('NotationRenderer', () => {
  ...
  describe('system wrapping (long pieces)', () => {
    ...
    // Per Gould "Behind Bars" (Systems chapter): every system must show
    // every voice. If a voice is silent for a whole system it gets a
    // whole-measure rest, but its staff still appears at the system's
    // synchronized vertical position so reading from one system to the
    // next stays continuous.
    it('renders every voice on every system, even when one voice is silent', () => {
      // Two-voice piece. Voice A (soprano) plays continuous quarter
      // notes. Voice B (alto) is silent (whole rests) for the first 4
      // measures, then plays in m5–m8. At width=400 we force ~2 measures
      // per system → 4 systems → in the first 2 systems voice B is
      // silent; in the last 2 systems voice A is silent (well, plays
      // less, doesn't matter — what matters is that the FIRST system
      // shows a voice-B staff at the same vertical offset relative to
      // voice A that we'd see in the system where voice B speaks).
      const sopranoNotes = [];
      const altoNotes = [];
      for (let m = 0; m < 8; m += 1) {
        for (let b = 0; b < 4; b += 1) {
        ...
        if (m < 4) {
          // whole rest fills the measure
          altoNotes.push({ length: '1/1' });
        } else {
          for (let b = 0; b < 4; b += 1) {
      ...
      ctx.render({
        ...
        keySignature: 'C',
        voices: [
          { id: 'soprano', clef: 'treble', notes: sopranoNotes },
          { id: 'alto', clef: 'treble', notes: altoNotes },
        ],
      });

      // Find every staff group, group by system index.
      const staves = Array.from(ctx.container.querySelectorAll('.staff'));
      const bySystem = new Map();
      staves.forEach((s) => {
        const idx = parseInt(s.getAttribute('data-system-index') || '0', 10);
        if (!bySystem.has(idx)) bySystem.set(idx, []);
        bySystem.get(idx).push(s);
      });

      // We MUST get at least 2 systems for the test to be meaningful.
      expect(bySystem.size).toBeGreaterThan(1);

      // EACH system must contain BOTH voices.
      const voiceIdsPerSystem = [...bySystem.entries()]
        .sort(([a], [b]) => a - b)
        .map(([, list]) => list.map((s) => s.getAttribute('data-voice-id')).sort());
      voiceIdsPerSystem.forEach((ids, sysIdx) => {
        expect(ids).toEqual(['alto', 'soprano']);
      });

      // The first system's alto staff must contain at least one rest
      // element (the whole-measure rest), so the silent voice is
      // visibly present rather than just an empty staff.
      const sys0AltoStaff = bySystem.get(0).find(
        (s) => s.getAttribute('data-voice-id') === 'alto',
      );
      const sys0AltoRests = sys0AltoStaff.querySelectorAll('.rest');
      expect(sys0AltoRests.length).toBeGreaterThan(0);
    });

    // Per Gould "Behind Bars" (Systems / Barlines): every system ends
    // with a barline. For non-final systems this is a regular thin
    // barline; the final system uses the existing final-barline
    // treatment. The end-of-system barline anchors the staff lines so
    // they don't trail past the music.
    it('ends every non-final system with a barline that the staff lines hit', () => {
      // 8 measures of quarter notes at width=800 — enough that the
      // renderer's per-measure spacing gives multiple measures per
      // system but the piece still wraps onto more than one system.
      const notes = [];
      for (let m = 0; m < 8; m += 1) {
        for (let b = 0; b < 4; b += 1) {
          notes.push({ pitch: 'C5', length: '1/4' });
      ...
      const staves = Array.from(ctx.container.querySelectorAll('.staff'));
      const sysCount = staves.length;
      expect(sysCount).toBeGreaterThan(1);

      // Sort by system index ascending.
      staves.sort((a, b) => (
        parseInt(a.getAttribute('data-system-index'), 10)
        - parseInt(b.getAttribute('data-system-index'), 10)
      ));

      // For every system EXCEPT the final one: the rightmost barline X
      // and the staff-line x2 should match (within 1px). And the
      // staff-line x2 should NOT extend out near the configured width
      // (400) — it should stop where the music actually ended.
      for (let i = 0; i < sysCount - 1; i += 1) {
        const staff = staves[i];
        const barLineEls = staff.querySelectorAll('.bar-line line');
        expect(barLineEls.length).toBeGreaterThan(0);
        const xs = [...barLineEls].map((l) => parseFloat(l.getAttribute('x1')));
        const rightmostBarX = Math.max(...xs);

        const staffLine = staff.querySelector('.staff-line');
        const lineX2 = parseFloat(staffLine.getAttribute('x2'));

        // Staff line ends at (within 1px of) the system-end barline.
        expect(Math.abs(lineX2 - rightmostBarX)).toBeLessThan(1.5);
        // And the system-end barline sits at-or-inside the configured
        // width (no trailing past the available rendering area). Allow
        // a small tolerance for the barline-padding allowance, but
        // catch the gross "trailing past width" defect.
        expect(rightmostBarX).toBeLessThanOrEqual(800 + 4);
      }
    });

    // For a wrapped multi-voice piece the inter-system gap (between
    // the LAST staff of system N and the FIRST staff of system N+1)
    // must be at least as large as the intra-system voice-to-voice
    // gap. Otherwise voices in successive systems read as belonging
    // to the same group (a single tightly-spaced quartet) instead of
    // separate systems — which is what was happening with Bach
    // Invention 1 at width=800 before this fix.
    it('separates systems vertically more than voices within a system', () => {
      const sopranoNotes = [];
      const altoNotes = [];
      for (let m = 0; m < 8; m += 1) {
        for (let b = 0; b < 4; b += 1) {
    ...
    // their shared system at the SAME barline X, otherwise the staves
    // look misaligned (one ending earlier than the other).
    it('aligns end-of-system barline X across voices within a system', () => {
      // Two voices, both continuous quarter notes, width forces wrap.
      const notesA = [];
      const notesB = [];
      for (let m = 0; m < 8; m += 1) {
        for (let b = 0; b < 4; b += 1) {
          notesA.push({ pitch: 'C5', length: '1/4' });
          notesB.push({ pitch: 'C4', length: '1/4' });
        }
      }
      ctx.renderer._width = 400;
      ctx.render({
        ...
        keySignature: 'C',
        voices: [
          { id: 'top', clef: 'treble', notes: notesA },
          { id: 'bot', clef: 'bass', notes: notesB },
        ],
      });
      ...
      // For each NON-final system: every voice's staff-line x2 must
      // agree (within 1px). This guarantees voices line up vertically
      // at the system-end barline.
      const sortedSysIdxs = [...bySystem.keys()].sort((a, b) => a - b);
      for (let i = 0; i < sortedSysIdxs.length - 1; i += 1) {
        const sysStaves = bySystem.get(sortedSysIdxs[i]);
        const x2s = sysStaves.map((st) => {
          const ln = st.querySelector('.staff-line');
          return parseFloat(ln.getAttribute('x2'));
        });
        const minX = Math.min(...x2s);
