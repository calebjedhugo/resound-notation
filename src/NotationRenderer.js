  ...
  render(songData) {
    ...
    if (systemPlans.length === 0) {
      ...
      for (let si = 0; si < systemPlans.length; si += 1) {
        ...
        // systemYOffset.
        const shiftedVoiceYPositions = voiceYPositions.map((y) => y + systemYOffset);
        ...
        // Advance Y for the next system.
        const systemBottom = voiceYPositions[voiceCount - 1] + STAFF_TOP_OFFSET + STAFF_HEIGHT + 40 + systemYOffset;
        lastSystemBottomY = systemBottom;
        systemYOffset += (voiceYPositions[voiceCount - 1] + STAFF_TOP_OFFSET + STAFF_HEIGHT + 40 - (voiceYPositions[0])) + SYSTEM_GAP;
      }
    }
